...
localparam BUZZER_HALF_PERIOD = 32'd12500; // 2kHz
        ...
        if (uart_rx_done) begin
            if (cfg_state == CFG_IDLE) begin
                if (uart_rx_data == 8'h53 || uart_rx_data == 8'h54 ||
                    ...
                    uart_rx_data == 8'h43 || uart_rx_data == 8'h47 ||
                    uart_rx_data == 8'h42) begin // 'S'/'T'/'L'/'H'/'M'/'R'/'E'/'N'/'C'/'G'/'B'
                ...
                if (uart_rx_data >= 8'h30 && uart_rx_data <= 8'h39) begin
                    ...
                    end else if (cfg_cmd == 8'h47) begin
                        if (cfg_value >= 32'd1 && cfg_value <= 32'd3000)
                            max_range_cfg <= cfg_value[15:0];
                    end else if (cfg_cmd == 8'h42) begin
                        if (cfg_value <= 32'd1)
                            buzzer_enable_cfg <= cfg_value[0];
                    end
                    cfg_state <= CFG_IDLE;
                end else begin
                    cfg_state <= CFG_IDLE;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (state)
                ...
                if (measure_done) begin
                    ...
                    if (multi_measure_en_cfg && multi_measure_count_cfg > 8'd1) begin
                        if (multi_meas_count + 1'b1 < multi_measure_count_cfg) begin
                            ...
                            send_state <= 4'd0;
                            digit_index <= 4'd0;
                            alarm_active <= ((multi_meas_sum + distance_mm) / multi_measure_count_cfg) <= max_range_cfg &&
                                            ((multi_meas_sum + distance_mm) / multi_measure_count_cfg) > 0;
                            state <= SEND_DISTANCE;
                        end
                    end else begin
                        state <= SEND_DISTANCE;
                        ...
                        send_state <= 4'd0;
                        digit_index <= 4'd0;
                        alarm_active <= (distance_mm <= max_range_cfg) && (distance_mm > 0);
                    end
                    ...
                    if (multi_measure_en_cfg && multi_measure_count_cfg > 8'd1) begin
                        if (multi_meas_count + 1'b1 < multi_measure_count_cfg) begin
                            ...
                            send_state <= 4'd0;
                            digit_index <= 4'd0;
                            alarm_active <= ((multi_meas_sum) / multi_measure_count_cfg) <= max_range_cfg &&
                                            ((multi_meas_sum) / multi_measure_count_cfg) > 0;
                            state <= SEND_DISTANCE;
                        end
                    end else begin
                        alarm_active <= 1'b0;
            ...
            default: state <= IDLE;
        endcase
        if (buzzer_enable_cfg && alarm_active) begin
            if (buzzer_counter >= BUZZER_HALF_PERIOD) begin
                buzzer_counter <= 32'd0;
                buzzer_out_reg <= ~buzzer_out_reg;
            end else begin
