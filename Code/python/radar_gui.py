...
class RadarGUI:
    ...
    def create_widgets(self):
        ...
        # 测距时间设置
        ttk.Label(param_frame, text="测距时间(s):").grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Button(param_frame, text="设置", command=self.set_measure_time).grid(row=1, column=2, padx=5, pady=5)
        
        # 旋转角度设置
        ttk.Label(param_frame, text="旋转角度(度):").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Button(param_frame, text="设置", command=self.set_angle_step).grid(row=2, column=2, padx=5, pady=5)
        
        # 扫描角度范围设置
        ttk.Label(param_frame, text="最小角度(度):").grid(row=3, column=0, sticky=tk.W, pady=5)
        ...
        min_angle_entry.grid(row=3, column=1, padx=5, pady=5)
        ttk.Button(param_frame, text="设置", command=self.set_min_angle).grid(row=3, column=2, padx=5, pady=5)
        
        ttk.Label(param_frame, text="最大角度(度):").grid(row=4, column=0, sticky=tk.W, pady=5)
        ...
        max_angle_entry.grid(row=4, column=1, padx=5, pady=5)
        ttk.Button(param_frame, text="设置", command=self.set_max_angle).grid(row=4, column=2, padx=5, pady=5)
        
        # 扫描模式设置
        ttk.Label(param_frame, text="扫描模式:").grid(row=5, column=0, sticky=tk.W, pady=5)
        ...
        self.scan_mode_combo.grid(row=5, column=1, padx=5, pady=5)
        ttk.Button(param_frame, text="设置", command=self.set_scan_mode).grid(row=5, column=2, padx=5, pady=5)
        ...
        self.multi_count_entry.grid(row=7, column=1, padx=5, pady=5)
        self.multi_count_btn = ttk.Button(param_frame, text="设置次数", command=self.set_multi_measure_count)
        self.multi_count_btn.grid(row=7, column=2, padx=5, pady=5)
        ...
        stability_hint.grid(row=8, column=1, sticky=tk.W, pady=5)
        self.attach_tooltip(
            stability_hint,
            "基于MAD(中位数绝对偏差)评估波动度，\n"
        ...
        # 伺服标定
        ttk.Label(param_frame, text="伺服标定:").grid(row=9, column=0, sticky=tk.W, pady=5)
        calib_frame = ttk.Frame(param_frame)
        calib_frame.grid(row=9, column=1, columnspan=2, sticky=tk.W, pady=5)
        ...
        clear_btn.grid(row=10, column=0, columnspan=3, pady=10)
    ...
    def angle_to_xy(self, angle, distance, center_x, center_y, max_range, radius):
        """将角度和距离转换为屏幕坐标（0-180度扇形）"""
        import math
        # 角度转换为弧度，0度向上，90度向右，180度向下
    ...
    def update_radar(self):
        ...
        # 绘制角度标记
        x0, y0 = self.angle_to_xy(0, radius + 15, center_x, center_y, self.max_range, radius)
        self.canvas.create_text(x0, y0, text="0°", fill="white", font=("Arial", 10))
        
        x90, y90 = self.angle_to_xy(90, radius + 15, center_x, center_y, self.max_range, radius)
        self.canvas.create_text(x90, y90, text="90°", fill="white", font=("Arial", 10))
        
        x180, y180 = self.angle_to_xy(180, radius + 15, center_x, center_y, self.max_range, radius)
        self.canvas.create_text(x180, y180, text="180°", fill="white", font=("Arial", 10))
        ...
        for angle, distance in self.distance_data:
            if distance > 0 and distance <= self.max_range:
                ...
                # 绘制红色点
                self.canvas.create_oval(x - 4, y - 4, x + 4, y + 4, fill="red", outline="red", width=1)
