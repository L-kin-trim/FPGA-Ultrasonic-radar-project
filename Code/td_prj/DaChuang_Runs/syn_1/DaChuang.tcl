...
set rc [catch {
  ...
  elaborate -top {radar_top_auto}
  export_db {DaChuang_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../DaChuang.adc
  optimize_rtl
  report_area -file DaChuang_rtl.area
  export_db {DaChuang_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea DaChuang_gate.area
  legalize_phy_inst
  export_db {DaChuang_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
